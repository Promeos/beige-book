...
from src.config import STATE_TO_DISTRICT, OUTPUT_DIR
...
def plot_sector_map_grid(sector_df, sectors=None, save=True):
    ...
    for i in range(len(sectors)):
        ...
        fig.update_layout(
            **{
                geo_key: dict(
                    scope="usa", showlakes=True, lakecolor="rgb(200, 220, 240)"
                ),
            }
        )
...
def plot_dominant_sector_map(sector_df, mode="strongest", save=True):
    ...
    district_sector = (
        df.groupby(["district", "sector"])["vader_compound"].mean().reset_index()
    )
    ...
    dominant_dict = dominant.set_index("district")[
        ["sector", "vader_compound"]
    ].to_dict("index")
